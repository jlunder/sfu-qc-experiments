...
mergeStructuralDuplicates inputGraph =
  ...
  where
    ...
    checkAndMergeNode s@(MergeState {mergeFalse = Just mID}) (XAG.Const nID False) =
      mergeNode nID mID s
    checkAndMergeNode s@(MergeState {mergeTrue = Nothing}) n@(XAG.Const nID True) =
      (keepNode n s) {mergeTrue = Just nID}
    checkAndMergeNode s@(MergeState {mergeTrue = Just mID}) (XAG.Const nID True) =
      mergeNode nID mID s
    checkAndMergeNode s (XAG.Not nID xID) =
    ...
    checkAndMergeNode s (XAG.Xor nID xID yID)
    ...
    checkAndMergeNode s (XAG.And nID xID yID)
