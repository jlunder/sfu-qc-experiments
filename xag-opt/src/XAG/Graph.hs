...
freeVariables allNodes = freeVariablesRev (reverse allNodes) IntSet.empty
  where
    ...
    freeVariablesRev (node : remain) freeSoFar =
      IntSet.delete
        (nodeID node)
        (freeVariablesRev remain (IntSet.union (nodeRefs node) freeSoFar))

-- compute the list of outputs possibly generated by this graph
outputs :: [Node] -> IntSet.IntSet
outputs gNodes = IntSet.fromList (map nodeID gNodes)
...
cover initialIDs allNodes = reverse (coverRev initialIDs (reverse allNodes))
  where
    coverRev _ [] = []
    coverRev coverIDs (node : remain) =
      if IntSet.member (nodeID node) coverIDs
        then node : coverRev (IntSet.union coverIDs (nodeRefs node)) remain
        else coverRev coverIDs remain
...
dominatingAnds ofID allNodes = dominatingAndsOfID (IntSet.singleton ofID) (skipToNode allNodes)
  where
    ...
    skipToNode [] = []
    skipToNode wholeList@(node : remain)
      | nodeID node > ofID = wholeList
      | otherwise = skipToNode remain

    dominatingAndsOfID :: IntSet.IntSet -> [Node] -> [(Int, (Int, Int))]
    dominatingAndsOfID _ [] = []
    dominatingAndsOfID searchSet (node : remain) =
      case node of
        -- Const nodes can't dominate anything
        Const _ _ -> dominatingAndsOfID searchSet remain
        -- Not: Add this node to the search list if it's controlled by anything in the search list
        Not nID xID -> dominatingAndsOfID (expandSearchIf (IntSet.member xID searchSet) nID) remain
        -- Xor: Add this node to the search list if it's controlled by anything in the search list
        Xor nID xID yID -> dominatingAndsOfID (expandSearchIf (IntSet.member xID searchSet || IntSet.member yID searchSet) nID) remain
        ...
        And nID xID yID -> withXYDoms
          where
            ...
            restOfDoms = dominatingAndsOfID searchSet remain
      where
...
validNodes = validNodesAux 0
  where
    ...
    validNodesAux nextID (node : remain) =
      validNode nextID node && validNodesAux (nodeID node + 1) remain
...
eval g@(Graph gNodes inIDs outIDs) inVec
  | not (valid g) = Nothing
...
instance QC.Arbitrary Graph where
  ...
  arbitrary = do
    ...
    gNodes <- mapM genNode [nVars .. nVars + nNodes - 1]
    randIns <- QC.shuffle [0 .. nVars - 1]
    let allOutputs = map nodeID gNodes
    outShuf <- QC.shuffle allOutputs
    randOuts <- QC.sublistOf outShuf
    return $ Graph {nodes = gNodes, inputIDs = randIns, outputIDs = randOuts}
    where
      genNode n = do
